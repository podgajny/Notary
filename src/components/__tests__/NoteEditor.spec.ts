...
describe('NoteEditor', () => {
  ...
  it('powinien mieć odpowiednie placeholdery w polach', () => {
    ...
    // Assert
    expect(wrapper.find('input[type="text"]').attributes('placeholder')).toBe('Title')
    expect(wrapper.find('textarea').attributes('placeholder')).toBe('Write your note...')
  })
  ...
  it('powinien wyświetlać błąd gdy zapisanie się nie powiedzie', async () => {
    // Arrange
    const error = new NoteStoreError('STORAGE_WRITE_FAILED', 'Could not save. Try again.')
    ...
    // Assert
    expect(wrapper.find('.text-red-600').text()).toBe('Could not save. Try again.')
  })
  ...
  it('powinien mieć referencję do pola tytułu', () => {
    ...
    // Assert
    expect(wrapper.vm.titleInputRef).toBeDefined()
  })
